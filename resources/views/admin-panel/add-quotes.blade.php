...
@section('content')
    <div class="bg-gray-400 rounded p-10 w-1/2 flex flex-col justify-center items-center m-auto mt-6">
        ...
        {{-- Edit Form --}}
        {!! Form::open(['action' => ['App\Http\Controllers\AdminController@addQuotes', $quote->id], 'method' => 'POST', 'class' => 'flex flex-col p-2 m-2']) !!}

        {{ Form::label('text', 'Quote', ['class' => 'mb-2']) }}
        {{ Form::text('quote', null, ['class' => 'p-2 border border-primary rounded mb-2 outline-none bg-indigo-50']) }}

        {{ Form::label('text', 'Quote Geo', ['class' => 'mb-2']) }}
        {{ Form::text('quoteGeo', null, ['class' => 'p-2 border border-primary rounded mb-2 outline-none bg-indigo-50']) }}
